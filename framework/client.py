...
class SAVNConnectionAssistant:
  ...
  def updateCarStates(self, simulationId, timestamp, state):
    factory.__proto__.sendMessage(json.dumps({'type': 'simulation-state', 'content': {'simulationId': simulationId, 'id': str(timestamp), 'timestamp': timestamp, 'objects': json.dumps(state)}}).encode('utf8'))
